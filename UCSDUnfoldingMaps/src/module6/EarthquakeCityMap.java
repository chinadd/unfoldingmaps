			...
			if (!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
				...
				for (Marker mhide : quakeMarkers) {
					...
					if (quakeMarker.getDistanceTo(marker.getLocation()) 
						...
						if ("Past Week".equals(age) || "Past Day".equals(age)){
							recent += quakeMarker.getTitle();
						}
					}
				}
				float average=(float)sum_mag/(float)count;
				String Average=Float.toString(average);
				String Count=Integer.toString(count);
				JOptionPane.showMessageDialog(frame, "Number of quakes:"+Count+"    "+
				"Average Magnitude:"+Average+"    "+"recent quakes:"+recent);
				return;
			}
		}		
	}
